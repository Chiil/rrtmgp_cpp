...
#### create data:
ncol = 1000
p_lay = np.zeros((228,ncol))
T_lay = np.zeros((228,ncol))
h2o_lay = np.zeros((228,ncol))
o3_lay = np.zeros((228,ncol))
for i in range(bg_len,228):
    for j in range(ncol):
        k = i-bg_len
        p_lay[i,j]   = sample_v(fullpres[k],fullpres[k+2])
        T_lay[i,j]   = sample_v_prof(p_lay[i,j],pres,T_min,T_max)
        h2o_lay[i,j] = sample_v_prof(p_lay[i,j],pres,q_min,q_max)

for i in range(228):
    for j in range(ncol):
        o3_lay[i,j]  = get_o3(p_lay[i,j])

h2o_lay = h2o_lay/(0.622-0.622*h2o_lay)

#### get t_lev:
T_lev = np.zeros((228+1,ncol))
...
nc_group_rad.createDimension("lay", 228)
nc_group_rad.createDimension("lev", 228+1)
